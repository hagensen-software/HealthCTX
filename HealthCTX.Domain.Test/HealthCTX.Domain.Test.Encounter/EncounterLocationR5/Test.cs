...
public class Test
{
    ...
    public void Encounter_FromFhirJsonGeneratesRecords()
    {
        ...
        Assert.Equal("http://terminology.hl7.org/CodeSystem/location-physical-type", encounter?.Locations.First().Form?.Codings.First().System?.Value.OriginalString);
        Assert.Equal("bu", encounter?.Locations.First().Form?.Codings.First().Code?.Value);
