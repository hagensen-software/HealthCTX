using HealthCTX.Domain.Test.Encounter.EncounterPriority;
...
public class Test
{
    ...
    public void Encounter_FromFhirJsonGeneratesRecords()
    {
        ...
        Assert.Equal("http://terminology.hl7.org/CodeSystem/service-type", encounter?.ServiceType?.Codings.First().System?.Value.OriginalString);
        Assert.Equal("26", encounter?.ServiceType?.Codings.First().Code?.Value);
    }
}
