using HealthCTX.Domain.Test.Observation.ObservationValueQuantity;
...
public class Test
{
    ...
    public void Observation_ToFhirJsonGeneratesJsonString()
    {
        ...
        var value = root
            .GetProperty("valueCodeableConcept")
            .GetProperty("coding")
            .EnumerateArray().First()
            .GetProperty("code");

        Assert.Equal("someCode", value.GetString());
    }
    ...
    public void Observation_FromFhirJsonGeneratesRecords()
    {
        ...
        Assert.Equal("someCode", observation?.Value.Coding.Code.Value);
    }
}
