...
namespace HealthCTX.Domain.Test.Observation.ObservationHasMember;
...
public class Test
{
    ...
    public void Observation_ToFhirJsonGeneratesJsonString()
    {
        var observation = new Observation(
            ...
            [new ObservationHasMember(
                new ObservationHasMemberReference("Observation/123"))]);
        ...
        var hasMember = root.GetProperty("hasMember").EnumerateArray().First();
        var reference = hasMember.GetProperty("reference");

        Assert.Equal("Observation/123", reference.GetString());
    }
    ...
    public void Observation_FromFhirJsonGeneratesRecords()
    {
        ...
        Assert.Equal("Observation/123", observation?.HasMember?.First().Reference.Value);
    }
}
