...
namespace HealthCTX.Domain.Test.Observation.ObservationComponentCode;
...
public class Test
{
    ...
    public void Observation_ToFhirJsonGeneratesJsonString()
    {
        ...
        var code = component.GetProperty("code");
        var coding = code.GetProperty("coding").EnumerateArray().First();

        Assert.Equal("8480-6", coding.GetProperty("code").GetString());
    }
    ...
    public void Observation_FromFhirJsonGeneratesRecords()
    {
        ...
        Assert.Equal("8480-6", observation?.Components.First().Code.Coding.Code.Value);
    }
}
