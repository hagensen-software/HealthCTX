...
namespace HealthCTX.Domain.Test.Persons.PersonPhoto;
...
public class Test
{
    ...
    public void Person_FromFhirJsonGeneratesRecords()
    {
        ...
        Assert.Equal("some mime type", person?.Photos.First().ContentType.Value);
        Assert.Equal(creationDate, person?.Photos.First().Creation.Value);
    }
}
